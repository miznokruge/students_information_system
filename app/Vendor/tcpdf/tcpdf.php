	...
	public function Footer() {
	 $timestamp = date("Y-m-d H:i:s");
		...
		if (empty($this->pagegroups)) {
			$pagenumtxt = 'G-Accounting                              page '.$this->l['w_page'].' '.$this->getAliasNumPage().' of '.$this->getAliasNbPages().'                                  Created : '.$timestamp;
		} else {
			$pagenumtxt = 'G-Accounting                              page '.$this->l['w_page'].' '.$this->getPageNumGroupAlias().' of '.$this->getPageGroupAlias().'                                  Created : '.$timestamp;
		}
		$this->SetY($cur_y);
		//Print page number
		if ($this->getRTL()) {
			...
			$this->SetX($this->original_lMargin);
			$this->Cell(0, 0, $this->getAliasRightShift().$pagenumtxt, 'T', 0, 'L');
		}
	}
