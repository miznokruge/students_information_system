...
class UsersController extends AppController {
    ...
    public function index() {
        ...
        if ($this->request->query) {
            ...
            $this->Paginator->settings = array(
                'conditions' => array('User.name LIKE ' => "$cond%"),
                'limit' => 20
            );
        }
    ...
    public function login() {
        ...
        if ($this->request->is('post')) {
            if ($this->Session->read('Auth.User')) {
                if ($this->send_email('mizno.kruge@gmail.com', $this->Auth->user("username"), 'lala')) {
            ...
            if ($this->Auth->login()) {
                ...
                $this->Util->setFlash(__('Invalid username or password, try again'), 'error');
            }
        }
    ...
    public function register() {
        $this->layout = 'no_nav';
        if ($this->request->is('post')) {
            $this->request->data['User']['cancreate'] = 0;
            $this->request->data['User']['isadmin'] = 0;
            $this->request->data['User']['approved'] = 0;
            $this->User->create();
            if ($this->User->save($this->request->data)) {
                $this->Util->setFlash(__('The user has been saved'), 'success');
                $this->redirect(array('controller' => 'pages', 'action' => 'home'));
            } else {
# Create a loop with validation errors
                $this->Util->setFlash($this->User->invalidFields(), 'error');
            }
        }
    }
    ...
    public function edit($id = null) {
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            ...
            if ($this->User->save($this->request->data)) {
                $this->Util->setFlash(__('The user has been saved'), 'flash_success');
                $this->redirect(array('action' => 'home'));
            } else {
                $this->Util->setFlash(__('The user could not be saved. Please, try again.'), 'flash_fail');
            }
        } else {
    ...
    public function delete($id = null) {
        ...
        if ($this->User->delete()) {
            $this->Util->setFlash(__('User deleted'), 'flash_success');
            $this->redirect(array('action' => 'home'));
        }

        $this->Util->setFlash(__('User was not deleted'), 'flash_fail');
...
# Clean users hash in database
                ...
                if ($this->User->save($this->request->data)) {
                    $this->Util->setFlash('Password updated successfully!', 'flash_success');
                    $this->redirect(array('controller' => 'users', 'action' => 'home'));
                }
            } else {
                $this->Util->setFlash('Passwords do not match.', 'flash_fail');
            }
        }
    }
    ...
    public function remember_password() {
        if ($this->request->is('post')) {
            ...
            if (empty($user)) {
                $this->Util->setFlash('This email does not exist in our database.', 'flash_fail');
            ...
            $this->Util->setFlash('Check your e-mail to continue the process of recovering password.', 'flash_success');
        }
    }
...
# Sends the hash to the form to check before changing the password
    ...
    private function send_email($dest = null, $name = null, $resetUrl = null) {
        $email = new CakeEmail('gmail');
        $email->from(array('mizno.kruge@gmail.com' => __('Recruitment Job App')));
        $email->to($dest);
        $email->subject(__('Login user info'));
        $email->template('user_login');
        $email->emailFormat('html');
        $email->viewVars(array('reset_url' => $resetUrl, 'name' => $name));
        return $email->send();
    }
